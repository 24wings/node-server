...
var version = 'v0.4.7',
    ...
    nmap = function(options, fn) {
        ...
        var tools = tools || {
            ...
            command: function(opts, block) {
                var flags = opts.flags.join(' '),
                    ipv6 = (validation.test(validation.net.IPv6, block)) ? ' -6 ' : ' ',
                    proto = (opts.udp) ? ' -sU' : ' ',
        ...
        var network = network || {
             ...
             * @returns {Array}
             */
            range: function(opts, host) {
                var blocks = cidr.list(host),
                    splitat = Math.round(blocks.length / opts.blocksize),
                    results = [],
                    tarray = [];
            ...
            calculate: function(opts) {
                ...
                if (tresults.length > 0) {
                    results = tools.flatten(tresults, results);
                }
